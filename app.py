...
def make_prediction():
    if request.method=='POST':
        ...
        if(re.match("keywords*",str(text).lower())!=None):
                output='KWD - '+text
                return render_template('index.html',output=output)
	if(re.match("introduction",str(text).lower())!=None or re.match("references",str(text).lower())!=None or re.match("conclusion",str(text).lower())!=None or 
	   re.match("case report.",str(text).lower())!=None or re.match("result.",str(text).lower())!=None or re.match("methods",str(text).lower())!=None or
	   re.match("methodology",str(text).lower())!=None):
		output='H1 - '+text
		return render_template('index.html',output=output)
